...
def index(requset):
    ...
    popular_items = Product.objects.filter(popular=1)
    ...
    return render(requset, 'main/index.html', context)
...
def test1(request, id):
    clock = Clock.objects.get(pk=id)
    clock_colors = clock.clockcolor_set.values_list('color__name', flat=True)
